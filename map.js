import {escapeHtml, formatNumber, geometryCentroid, normalize} from './utils.js';
...
export class ParcelMap{
  constructor({onSelect,onToast,onPointPlaced}){
    ...
    this.map=null;this.layers={};this.baseLayer=null;this.lastState={parcelles:[],points:[]};this.selectedId=null;this.watchId=null;this.gpsMarker=null;this.colorMode='culture';this.rpgVisible=false;this.rpgData=null;this.pointPlacementHandler=null;this.polygonDraw=null;this.unavailable=false;
  }
  ...
  init(){
    ...
    this.layers.parcels=L.layerGroup().addTo(this.map);this.layers.points=L.layerGroup().addTo(this.map);this.layers.rpg=L.layerGroup().addTo(this.map);this.layers.gps=L.layerGroup().addTo(this.map);this.layers.drawing=L.layerGroup().addTo(this.map);
  ...
  dispose(){if(this.watchId!==null)navigator.geolocation.clearWatch(this.watchId);this.cancelPointPlacement();this.cancelPolygonDrawing(false);}
}
