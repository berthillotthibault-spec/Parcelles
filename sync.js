...
function localDeviceId(){const key='parcelles:device-id';let id=localStorage.getItem(key);if(!id){id=uid('device');localStorage.setItem(key,id);}return id;}
...
export class SyncService{
  ...
  async heartbeat(){if(!this.user||!this.db||!this.workspaceId)return;this.deviceId=this.deviceId||localDeviceId();await this.db.collection('workspaces').doc(this.workspaceId).collection('devices').doc(this.deviceId).set({deviceId:this.deviceId,name:localStorage.getItem('parcelles:device-name')||defaultDeviceName(),userId:this.user.uid,email:emailKey(this.user.email),lastSeen:Date.now(),build:BUILD_ID,userAgent:String(navigator.userAgent||'').slice(0,300)},{merge:true});}
  async renameDevice(name){const value=String(name||'').trim();if(!value)throw new Error('Nom requis.');localStorage.setItem('parcelles:device-name',value);await this.heartbeat();await this.audit('device-rename','device',this.deviceId,{name:value});}
